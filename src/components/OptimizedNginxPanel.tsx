...
const OptimizedNginxPanel = memo(function OptimizedNginxPanel() {
  const { vhosts, loading, loadVhosts, createVhost, removeVhost } = useVirtualHosts('nginx');
  ...
  const handleReload = useCallback(async () => {
    ...
    if (result.success) {
      addToast('Nginx reloaded', 'success');
    } else {
      addToast(result.error || 'Failed to reload', 'error');
    }
  }, [addToast]);
      ...
      <div className="glass-card-premium p-6 mb-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-100">Nginx Virtual Hosts</h2>
              <p className="text-sm text-slate-400">Manage Nginx configurations</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              disabled={loading}
              className="p-3 rounded-xl bg-slate-800/50 border border-slate-600/50 text-slate-300 hover:text-emerald-400 transition-colors"
            >
              ...
              <div className="col-span-6">
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="p-3 rounded-lg bg-slate-800/50 border border-slate-600/50 text-slate-400 hover:text-emerald-400 transition-colors"
                  >
        ...
        <div className="overflow-auto h-full scrollbar-thin">
          <table className="w-full">
            <thead className="sticky top-0 z-10 bg-slate-900/80 backdrop-blur">
              <tr className="text-left text-xs font-medium text-emerald-500 uppercase tracking-wider">
...
export default OptimizedNginxPanel;
